document.addEventListener('DOMContentLoaded', function () {
  ...
  function ensureThemeLink() {
    ...
    if (!link) {
      link = Array.from(document.querySelectorAll('link[rel="stylesheet"]'))
        .find(l => /styles(-light)?\.css$/.test((l.getAttribute('href')||'').split('?')[0]));
    }
    if (!link) {
  ...
  // Initialize theme from localStorage or document dataset
  const savedTheme = localStorage.getItem('site-theme');
  const initialTheme = savedTheme || document.documentElement.dataset.theme || 'dark';
  const themeLink = ensureThemeLink();
  themeLink.href = initialTheme === 'light' ? 'assets/css/styles-light.css' : 'assets/css/styles.css';
  document.documentElement.dataset.theme = initialTheme;
  updateThemeButtons(initialTheme);

  // Enable transitions after page load to avoid entrance animations
  ...
  if (themeButtons.length) {
    themeButtons.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        link.href = selected === 'light' ? 'assets/css/styles-light.css' : 'assets/css/styles.css';
  ...
  let quizTask1Complete = false, quizTask1Correct = false;
  let quizTask2Complete = false, quizTask2Correct = false;
  let quizTask3Complete = false, quizTask3Correct = false;
  ...
  const updateQuizTotalResult = () => {
    ...
    const message = score === 3 ? 'Молодец! Ты успешно прошёл викторину.' : 
                    score >= 1 ? 'Хороший результат. Можно стать ещё внимательнее.' : 
                    'Стоит повторить правила безопасности и попробовать снова.';
    quizTotalResult.hidden = false;
    quizTotalResult.innerHTML = `<h4>Общий результат</h4><p>Правильных ответов: <strong>${score}</strong> из 3.</p><p>${message}</p>`;
  };
    ...
    function setupDragZone(zone) {
      ...
      dropWrapper.addEventListener('drop', (e) => {
        ...
        if (isCorrect) {
          ...
          if (dragScore === phishingCards.length) { quizTask1Complete = true; quizTask1Correct = true; updateQuizTotalResult(); }
        } else {
  ...
  if (compareGame) {
    ...
    let compareScore = 0, compareAnswered = 0;
    ...
    comparePairs.forEach(pair => {
      ...
      row.querySelectorAll('.compare-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          if (compareAnswered === comparePairs.length) { quizTask2Complete = true; quizTask2Correct = (compareScore === comparePairs.length); updateQuizTotalResult(); }
        });
      });
  ...
  if (scenarioQuiz) {
    ...
    scenarioQuiz.querySelectorAll('.quiz-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        scenarioResult.innerHTML = `<h4>Результат</h4><p>${correct ? 'Правильно! Это самое безопасное действие.' : 'Неверно. Выберите вариант, который не позволит атакующим получить доступ к аккаунту.'}</p>`;
  ...
  if (passwordInput) {
    ...
    generatePasswordBtn?.addEventListener('click', () => { const p = generatePassword(); passwordInput.value = p; updateMeter(p); checkCriteria(p); });
    clearPasswordBtn?.addEventListener('click', () => { passwordInput.value = ''; updateMeter(''); checkCriteria(''); });

    renderCriteria(); 
    updateMeter(passwordInput.value); checkCriteria(passwordInput.value);
  }
  ...
  function initScreenshotLightbox() {
    ...
    const updateLightbox = () => {
      ...
      if (!item) return;
      imgElement.src = item.src; imgElement.alt = item.alt || '';
      counter.textContent = gallery.length > 1 ? `${currentIndex + 1} / ${gallery.length}` : '';
      content.classList.toggle('has-controls', gallery.length > 1);
    };

    const closeLightbox = () => { overlay.classList.add('hidden'); document.documentElement.style.overflow = ''; };
    const openLightbox = () => { overlay.classList.remove('hidden'); document.documentElement.style.overflow = 'hidden'; updateLightbox(); };

    const visibleImages = (container) => Array.from(container.querySelectorAll('img')).filter(img => {
      const sc = img.closest('.screenshot');
      return sc && window.getComputedStyle(sc).display !== 'none';
    });
    ...
    prevBtn.addEventListener('click', () => { currentIndex = (currentIndex - 1 + gallery.length) % gallery.length; updateLightbox(); });
    nextBtn.addEventListener('click', () => { currentIndex = (currentIndex + 1) % gallery.length; updateLightbox(); });
    window.addEventListener('keydown', (e) => {
      if (overlay.classList.contains('hidden')) return;
      if (e.key === 'Escape') closeLightbox();
      if (e.key === 'ArrowLeft' && gallery.length > 1) { currentIndex = (currentIndex - 1 + gallery.length) % gallery.length; updateLightbox(); }
      if (e.key === 'ArrowRight' && gallery.length > 1) { currentIndex = (currentIndex + 1) % gallery.length; updateLightbox(); }
    });
  }
  ...
  if (checklistContainer) {
    ...
    const updateFinishButton = () => {
      ...
      finishButton.disabled = !(checked === items.length);
      finishButton.classList.toggle('btn-success', checked === items.length);
      finishButton.classList.toggle('btn-disabled', checked !== items.length);
    };
    ...
    finishButton?.addEventListener('click', () => { if (!finishButton.disabled) window.location.href = 'index.html'; });

    items.forEach(item => {
      const cb = item.querySelector('input');
      const stepId = item.dataset.stepId;
      const wasChecked = savedState[stepId] === true || savedState[stepId] === 'true';
      cb.checked = wasChecked;
      item.classList.toggle('completed', wasChecked);

      const toggle = () => { cb.checked = !cb.checked; item.classList.toggle('completed', cb.checked); cb.dispatchEvent(new Event('change')); };
  ...
  if (quizForm) {
    ...
    const showResult = (score) => {
      ...
      const message = score === total ? 'Отлично! Вы ответили правильно на все вопросы.' : 
                      score >= total - 1 ? 'Хорошо! Почти всё правильно.' : 'Постарайтесь внимательнее — безопасность важна.';
      resultBox.innerHTML = `<h4>Результат</h4><p>Правильных ответов: <strong>${score}</strong> из ${total}.</p><p>${message}</p>`;
    };
    ...
    quizForm.addEventListener('change', () => { if (!resultBox.hidden) resultBox.hidden = true; });
  }
});